...
class DbDataHandler:
    ...
    async def fetch_report_from_db_cache(self,imsi: str) -> dict | None:
        self.collections = self.db[settings.cache_collection_name]
    ...
    async def fetch_mapping_from_msisdn_to_imsi(self,msisdn: str) -> dict | None:
        self.collections = self.db[settings.map_msisdn_imsi_collection_name]
        return await self.collection.find_one({"msisdn": msisdn})
    
    async def fetch_mapping_from_cell_id_to_polygon(self,cell_id: str) -> dict | None:
        self.collections = self.db[settings.map_cellId_to_polygon_collection_name]
